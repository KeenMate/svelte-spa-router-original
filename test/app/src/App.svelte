...
<ul class="navigation-links">
	...
	<li><a
		href="/hello/svelte"
		use:link
		use:active={{path: '/hello/*', className: 'active another-class', inactiveClassName: 'inactive'}}
	>Say hi!</a></li>
...
<ul class="navigation-dynamic-links">
	{#each dynamicLinks as dl (dl.id)}
		<li>
			...
			-
			<i role="button" id="delete-link-{dl.id}" on:click={() => dynamicLinks = dynamicLinks.filter(e => e.id != dl.id)}>delete
				link</i>
		</li>
	{/each}
</ul>
...
<script>
	...
	import Router, {link, location, params, pop, push, querystring, replace} from "../../../Router.svelte"
